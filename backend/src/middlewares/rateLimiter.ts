...
const makeLimiter = ({ windowMs, max, message }: LimiterConfig): RateLimitRequestHandler =>
  rateLimit({
    windowMs,
    max,
...
const loginLimiter = makeLimiter({
  ...
  max: env.RATE_LIMIT_LOGIN_MAX || 10,
  message: "Too many login attempts. Please try again later.",
});

const otpVerifyLimiter = makeLimiter({
  windowMs: env.RATE_LIMIT_OTP_VERIFY_WINDOW_MS || 15 * 60 * 1000,
  max: env.RATE_LIMIT_OTP_VERIFY_MAX || 10,
  message: "Too many verification attempts. Please try again later.",
});

const otpResendLimiter = makeLimiter({
  windowMs: env.RATE_LIMIT_OTP_RESEND_WINDOW_MS || 15 * 60 * 1000,
  max: env.RATE_LIMIT_OTP_RESEND_MAX || 5,
  message: "Too many OTP requests. Please try again later.",
});

const complaintLimiter = makeLimiter({
  windowMs: env.RATE_LIMIT_COMPLAINT_WINDOW_MS || 60 * 60 * 1000,
  max: env.RATE_LIMIT_COMPLAINT_MAX || 20,
  message: "Too many complaints submitted. Please try again later.",
});

const pcSyncLimiter = makeLimiter({
  windowMs: env.RATE_LIMIT_PC_SYNC_WINDOW_MS || 60 * 1000,
  max: env.RATE_LIMIT_PC_SYNC_MAX || 30,
